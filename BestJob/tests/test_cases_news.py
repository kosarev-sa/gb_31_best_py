...
class TestNews:
    ...
    def test_insufficient_create_news(self, moderator_client, news, image):
        with allure.step('Создаем новость'):
            try:
                moderator_client.post(self.path_create,  data=news)
            except AttributeError:
                pass
            else:
                assert False, 'Новость была создана'
    ...
    def test_invalid_create_news(self, moderator_client, news, image):
        with allure.step('Создаем новость'):
            try:
                moderator_client.post(self.path_create,  data=news)
            except AttributeError:
